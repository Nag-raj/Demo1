...
function onLoadServiceIFrame()
{
    var strWhere = "onLoadServiceIFrame()";
	...
	try
	{
		$XIFrame.onload({
			...
			onInvalidHandler: onLoadServiceInvalidHandler,
			onError: onLoadServiceError
		});
    }
    catch (ex)
    {
        iFrame.className = "hostedIframe";
        alert(strWhere +"error message : " +  ex);
    }
}


function onLoadServiceSuccess(msg)
{
	console.log('A form for the merchant guid and access token combination is loading in the iFrame successfully.');
}

function onLoadServiceInvalidHandler(e)
{
	alert("onLoadServiceInvalidHandler() : "+ e);
}

function onLoadServiceError(e)
{
	alert("onLoadServiceError() : " + e);
}
...
function submitServiceForm()
{
    ...
    if(iframe)
    {
		$XIFrame.submit({
			iFrameId: 'xieCommFrame', 
			targetUrl: iframe[0].getAttribute("src"),
			onSuccess: onSubmitServiceSuccess, 
			onError: onSubmitServiceError
			});
    }
	
}
function onSubmitServiceSuccess(msg)
{
...
function onSubmitServiceError(msg)
{
	alert("onSubmitServiceError(): " + msg);
